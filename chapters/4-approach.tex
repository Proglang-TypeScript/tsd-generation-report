...
The \mintinline{bash}{operator} interaction stores the operator being used and the type of the left hand and right hand operands. It also indicates which operand is the one that triggered the interaction. The supported operators are the ones that trigger Jalangi's \mintinline{bash}{binaryPre()} callback. Conditionals are merged into one operator name \mintinline{bash}{conditional}, since Jalangi's \mintinline{bash}{conditional()} callback does not offer a way of identifying the operator for this cases. Both callbacks are explained in section \secref{sec:jalangi}.

\figref{fig:run-time-information-gathering-operator} provides an example for operator \mintinline{bash}{+}.
