...
\begin{code}
  ...
  \caption[Modified accessed property]{\textbf{Modified accessed property} - Property \mintinline{text}{bar} gets accessed twice. Two interactions are saved.}
...
$ docker run --name generate-declaration-file \
  ...
  tsd-generator --module-name calculator -i /tmp/output.json
...
Before comparison, declaration files need to be parsed so that both of them share the same structure. It was achieved by using the TypeScript Compiler API, a library developed by Microsoft that allows to traverse the Abstract Syntax Tree in an easy and intuitive way \citep{typescript-compiler-api}.
