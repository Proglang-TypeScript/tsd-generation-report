...
\item A framework that extracts code examples from the
  documentation of an NPM modules and collects run-time type
...
\item An evaluation of our framework. We examined all 6029 entries in
  ...
  well-documented NPM libraries, on which we ran \texttt{dts-generate}
...
has about 35000 weekly downloads and 221 NPM packages depend on it. If
...
file. The result is a valid declaration file
shown in \figref{fig:motivating-example-route-parser-vscode}  that 
can be used in a TypeScript project. For example,  Visual Studio
Code performs code completion properly
(\figref{fig:motivating-example-route-parser-vscode}). If the
\texttt{route-parser} package gets modified in a future, a new declaration
...
The command line interface was inspired in the \texttt{dts-gen} \cite{dts-gen} package. It can be seen in \coderef{code:dts-generate-example} that invoking the package is very simple and the only required argument is the name of the module published to the npm registry.
...
\begin{itemize}
  \item Obtain the repository url from the package: The command \texttt{npm view <PACKAGE> repository.url} can be used for retrieving the url of the package's repository.
...
export = escapeHTML;
    ...
    \caption{\textbf{Module-function results} - Results are shown for modules \texttt{abs}, \texttt{dirname-regex}, \texttt{escape-html}. On the left side the generated declaration file with \lstinline{dts-generate}. On the right side the corresponding file in the DefinitelyTyped repository. Functions are correctly detected and input types are accurately inferred. Both files are parsed for comparison, as explained in \secref{sec:dts-generate-evaluation}. Therefore, subtle differences in the syntax between both files are not important.}
\end{figure}

Templates of type \lstinline{module-class} are shown for modules \texttt{flake-idgen}, \texttt{route-parser} and \texttt{timer-machine} in \figref{fig:experiments-results-module-class-flake-idgen} and \figref{fig:experiments-results-module-class-timer-machine}, respectively. Properties of interfaces and class methods are correctly generated. Optional parameters are not detected, as it was not considered for the implementation. Finally, \lstinline{module} template is presented for \texttt{is-uuid} module in \figref{fig:experiments-results-module-is-uuid}.
...
function BufferedMetricsLogger(opts) {
    ...
    \caption{\textbf{Missing properties | datadog-metrics} - Properties \texttt{aggregator} and \texttt{reporter} are not in the DefinitelyTyped version, but they appear in the generated declaration file. However, they are indeed used by the library, as exposed in lines 2 and 3 of the library's source code shown in \protect\subref{fig:subfloat-datadog-logger}.}
